...
#     Reads source agent templates (default: <oskr_root>/.claude/agents/),
#     substitutes {{PROJECT_NAME}} and {{TECH_STACK}} from harness-config.json's
#     agent_context block, and writes the rendered .md files into <dest_dir>.
#     Override the source directory via HARNESS_AGENTS_SOURCE.
...
harness_render_agents() {
  local dest_dir="$1"
  local source_dir="${HARNESS_AGENTS_SOURCE:-$(_harness_oskr_root)/.claude/agents}"
  local project_name tech_stack
  project_name=$(harness_config_get '.agent_context.project_name') || return 1
  tech_stack=$(harness_config_get '.agent_context.tech_stack') || return 1

  [[ -n "$dest_dir" ]] || { _harness_die "harness_render_agents: dest_dir required"; return 1; }
  [[ -d "$source_dir" ]] || { _harness_die "agent source dir not found: $source_dir"; return 1; }
  mkdir -p "$dest_dir"
    ...
    PROJECT_NAME="$project_name" TECH_STACK="$tech_stack" \
      awk '
        { gsub(/\{\{PROJECT_NAME\}\}/, ENVIRON["PROJECT_NAME"]);
          gsub(/\{\{TECH_STACK\}\}/, ENVIRON["TECH_STACK"]);
          print }
      ' "$f" > "$dest_dir/$base"
    count=$((count + 1))
  done
  ...
  [[ "$count" -gt 0 ]] || { _harness_die "no agent templates found in $source_dir"; return 1; }
  printf '%s' "$count"
}
