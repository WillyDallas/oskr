...
HARNESS_CONFIG="$REPO_ROOT/tests/scripts/fixtures/harness-config.sample.json" \
  bash -c "source '$REPO_ROOT/bin/harness-lib.sh' && [[ \$(harness_config_get '.github.owner') == 'WillyDallas' ]]"
...
TEST2_OUT=$(
  cd "$TMPDIR" && \
    HARNESS_CONFIG="" bash -c "source '$REPO_ROOT/bin/harness-lib.sh' && harness_config_path" 2>&1 \
  || true
)
...
HARNESS_CONFIG="$REPO_ROOT/tests/scripts/fixtures/harness-config.malformed.json" \
  bash -c "source '$REPO_ROOT/bin/harness-lib.sh' && harness_config_get '.github.owner'" 2>/dev/null \
