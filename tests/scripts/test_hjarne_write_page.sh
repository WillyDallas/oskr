...
# create → v1 + today on line 2, title preserved on line 1, Mode: deep by default
...
grep -qF 'Mode: deep' <<<"$L2" || { echo "FAIL: default Mode not deep ($L2)" >&2; exit 1; }
...
# explicit mode arg → v3 + Mode: quick (schema.md: '> Written <date> · Mode: <deep|quick> · v<N>')
hjarne_write_page "$PAGE" "$CONTENT" quick
L2=$(sed -n 2p "$PAGE")
grep -qF 'v3' <<<"$L2" || { echo "FAIL: mode update not v3 ($L2)" >&2; exit 1; }
grep -qF 'Mode: quick' <<<"$L2" || { echo "FAIL: explicit Mode not stamped ($L2)" >&2; exit 1; }
